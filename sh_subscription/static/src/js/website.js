...
publicWidget.registry.business_subscription_product = publicWidget.Widget.extend({
    ...
    events: {
                // auto display server specifications based on workers numbers
    ...
    init() {
        ...
        $("input[data-attribute_name=Workers]").closest('li').find('.badge .oe_currency_value').each(function(){
            var price = $(this).text();  
    ...
    handle_server_specifications(event)
    {
        var o_variant_pills = $(event.currentTarget);
            ...
            if(variant.attr('data-attribute_name') == 'Workers')
            {
                ...
                $("li[data-attribute_name=SSD] select option").removeAttr('selected');
                if(variant.attr('data-value_name')=='1')
                {
                    $("li[data-attribute_name='Processor'] select option[data-value_name='2 Cores']").prop('selected', 'selected');
                    $("li[data-attribute_name='RAM'] select option[data-value_name='4Gi']").prop('selected', 'selected');
                    $("li[data-attribute_name='SSD'] select option[data-value_name='40Gi']").prop('selected', 'selected');
                }
                else if(variant.attr('data-value_name')=='2')
                {
                ...
                else if(variant.attr('data-value_name')=='3')
                {
                ...
                else
                {
                    ...
                    $("li[data-attribute_name='SSD'] select option[data-value_name='320Gi']").prop('selected', 'selected');
                }
            }
    }
});
