...
class account_move(models.Model):
    ...
    def action_post(self): 
        ...
        action_post = super().action_post()
        for record in self:
            if record.company_id.meta_invoice_confirmation:
                if record.state == 'posted':
                    model_id = record.env['ir.model'].search_read([('model','=','account.move')],['id','model'], limit=1)[0]
                    
                    filter = [  
                                ('name','=','whatsapp_invoice_sending'),
                    ...
                    _payload = {
                                    ...
                                    'meta_header_title': _('Invoice Confirmed'),  

                                    'odoo_record_amount_total': str(format_amount(self.env, record.amount_total, record.currency_id)),
