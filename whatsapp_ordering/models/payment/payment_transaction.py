...
class payment_transaction(models.Model):
    ...
    def _set_done(self):   
        ...
        for record in self:
            if record.company_id.meta_payment_transaction_confirmation:
                    if record.state == 'done':
                        base_url = self.env['ir.config_parameter'].sudo().get_param('web.base.url')
                        sharable = record.sale_order_ids._get_share_url(redirect=True)
                        url_params = parse_qs(sharable)
                        url_sharable = str(base_url) + str('/my/orders/') + str(record.sale_order_ids.id) + str('?access_token=') + str(url_params['access_token'][0])
                        if not url_sharable:
                            url_sharable = str('N/A')

                        model_id = record.env['ir.model'].search_read([('model','=','payment.transaction')],['id','model'], limit=1)[0]

                        filter = [  
                                    ('name','=','whatsapps_sales_payment_confirmation'),
                        ...
                        _payload = {
                                    ...
                                    'meta_header_title': _('Payment Receipt'),     
                                    ...
                                    'odoo_record_name':record.reference,
                                    'odoo_preview_url':'http://localhost',
                                    'odoo_record_partner_name': record.partner_id.name,

                                    'odoo_record_amount': str(format_amount(self.env, record.amount, record.currency_id)),
                                    'odoo_record_payment_method': str(record.payment_method_id.name),
                                    'odoo_record_payment_document_origin': str(record.sale_order_ids.name),                                    
                                    }
                        ...
                        if 'error' in response_values:                    
                            values = {
                                        ...
                                        'record_name':str(record.reference),
